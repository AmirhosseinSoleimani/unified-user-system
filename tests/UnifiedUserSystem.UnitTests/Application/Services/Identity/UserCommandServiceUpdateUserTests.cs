...
{
    [Fact]
    public async Task UpdateUserAsync_Should_UpdateProfileFields_When_FirstNameAndLastNameAreProvided()
    {
        // Arrange
        ...
        result.LastName.Should().Be("Fullname");
    ...
    [Fact]
    public async Task UpdateUserAsync_Should_WriteAuditLogWithProfileOldAndNewValues_When_FirstNameAndLastNameChange()
    {
        // Arrange
        var now = new DateTimeOffset(2024, 01, 01, 10, 00, 00, TimeSpan.Zero);
        var actorUserId = Guid.NewGuid();
        ...
        capturedRequest.OldValues!["FirstName"].Should().Be("Alice");
        capturedRequest.NewValues!["FirstName"].Should().Be("Updated");
        capturedRequest.OldValues!["LastName"].Should().Be("Doe");
        capturedRequest.NewValues!["LastName"].Should().Be("Fullname");
        capturedRequest.OldValues!.Keys.Should().BeEquivalentTo(
            "Fullname",
            "FirstName",
            "LastName");

        capturedRequest.NewValues!.Keys.Should().BeEquivalentTo(
            "Fullname",
            "FirstName",
            "LastName");
    }
    ...
    public async Task UpdateUserAsync_Should_WriteOnlySafeChangedFields_When_MultipleFieldsChange()
    {
        ...
        capturedRequest.OldValues!.Should().ContainKey("FirstName");
        capturedRequest.OldValues.Should().ContainKey("LastName");
        capturedRequest.OldValues.Should().ContainKey("PhoneNumber");
        capturedRequest.OldValues.Should().ContainKey("Username");
        capturedRequest.OldValues.Should().ContainKey("PasswordChanged");
        capturedRequest.NewValues!.Should().ContainKey("FirstName");
        capturedRequest.NewValues.Should().ContainKey("LastName");
        capturedRequest.NewValues.Should().ContainKey("PhoneNumber");
        ...
        capturedRequest.OldValues["LastName"].Should().Be("Doe");
        capturedRequest.NewValues["LastName"].Should().Be("Fullname");
        capturedRequest.OldValues["PhoneNumber"].Should().Be("+989123456789");
        capturedRequest.NewValues["PhoneNumber"].Should().Be("09123456789");
        capturedRequest.OldValues["Username"].Should().Be("alice");
        capturedRequest.NewValues["Username"].Should().Be("updateduser");
        capturedRequest.OldValues["PasswordChanged"].Should().Be(false);
        capturedRequest.NewValues["PasswordChanged"].Should().Be(true);
    ...
    public async Task UpdateUserAsync_Should_NotWriteAuditLog_When_UserIsNotFound()
    {
        ...
        // Act
        Func<Task> act = async () => await sut.UpdateUserAsync(userId, new UpdateUserRequest { FirstName = "Updated" }, ct);
